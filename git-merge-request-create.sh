...
usage() {
  echo
  echo "Usage:"
  echo " ${0##*/} [-p <PROJECT NAME>] [-s <SOURCE BRANCH>] [-t <TARGET BRANCH>] [-m <TITLE>] [-d <DISABLE AUTO PUSH>] [-v] [-h]"
  echo
  echo "Options:"
  echo " -p: default current project"
  echo " -s: default current branch"
  echo " -t: default master branch"
  echo " -m: default latest commit"
  echo " -d: default auto push current branch to origin branch, if set disable auto push"
  echo " -v: show version"
  echo " -h: show help"
  echo
  echo "Example 1:"
  echo " ${0##*/} -p bell/owl-ios -s test -t master -m title -d"
  echo "Example 2"
  echo " ${0##*/}"
  echo
}
...
checkAccessToken() {
  if [[ -z ${GITLAB_ACCESS_TOKEN} ]]; then
    ...
    echo You can generate Gitlab Access Token from Gitlab > Settings > Access Tokens
    exit 1
  fi
}
...
version=0.0.5
while getopts 'p:s:t:m:d:vh' arg
do
  case $arg in
